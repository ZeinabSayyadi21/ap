...
public class Main_EX3_LM_2_1 {
    public static void main(String[] args) {
        ...
        Safekeeping[] loadedSafekeepings = loadSafekeepingFromFile("safekeeping.txt");
        System.out.println("\nLoaded safekeeping Books:");
        for (Safekeeping safekeeping : loadedSafekeepings) {
            safekeeping.showList();
        }
    }
    ...
    private static void saveInitialData() {
        ...
        Safekeeping[] safekeepings = {
                new Safekeeping("463132", "Computer Networks", "1404-1-23", "1404-2-13"),
                new Safekeeping("463133", "Database", "1404-1-18", "1404-2-8")
        ...
        saveSafekeepingToFile(safekeepings, "safekeeping.txt");
    }
    ...
    private static Safekeeping[] loadSafekeepingFromFile(String fileName) {
        ArrayList<Safekeeping> safekeepingList = new ArrayList<>();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length == 4) {
                    Safekeeping safekeeping = new Safekeeping(
                            ...
                            parts[2].trim(),
                            parts[3].trim());
                    safekeepingList.add(safekeeping);
                }
            }
        } catch (IOException e) {
            System.out.println("Error reading safekeeping from file!" + e.getMessage());
        }
        return safekeepingList.toArray(new Safekeeping[0]);
    }
    ...
    private static void saveSafekeepingToFile( Safekeeping[] safekeepings, String fileName) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
            for (Safekeeping safekeeping : safekeepings ) {
                writer.println(safekeeping.toFileString());
            }
        } catch (IOException e) {
            System.out.println("Cannot save safekeeping to file!");
        }
    }
    ...
    public static class Safekeeping {
        ...
        Safekeeping( String id, String bookTitle, String trustDate, String returnDate) {
            this.id = id;
        ...
        void showList() {
            System.out.println("Student id: " + id + " | Book title: " + bookTitle + " | Trust date: " + trustDate +
                    " | Return Date: " + returnDate );
        }

        String toFileString() {
            return id + "," + bookTitle + "," + trustDate + "," + returnDate + ",";
        }
    }
